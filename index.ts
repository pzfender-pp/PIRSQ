...
*/
function findTargetSum(nums: number[], target: number): boolean {
...
*/
function sortingNumber(nums: number[]): number[] {
...
function findMaxPossibleValue(nums: number[]) {
    return nums
        .sort((a, b) => {
...
function findSumOfPrimeNumber(n: number): number {
    let sum = 0;
